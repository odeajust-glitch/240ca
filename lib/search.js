const STOPWORDS = new Set([
  ...
  'if','not','no','than','then','do','does','did','i','you','he','she',
...
// Fold simple plurals so "claims" matches "claim" and "duties" matches
// "duty". Deliberately crude — it runs identically at index and query
// time, so consistent folding matters more than linguistic accuracy.
// Runs after the stopword filter so words like "does" are dropped as
// stopwords rather than folded into non-words.
function foldPlural(t) {
  if (/\d/.test(t)) return t; // numeric/compound tokens (article18, 4.16) stay as-is
